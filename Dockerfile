...
# Хеш коммита, из которого собран образ: приложение отдаёт его в ответе /health,
# и выкатка по нему понимает, что поднялась именно новая версия.
#
# Стоит в самом конце файла намеренно: значение меняется при каждой сборке, и
# всё, что написано ниже такой строки, пересобирается заново. Ниже только
# ENTRYPOINT — он не создаёт слоя, поэтому тяжёлые слои с зависимостями и
# собранным кодом остаются общими для соседних версий образа.
ARG APP_COMMIT=dev
