...
- **Интеграционные тесты:** отдельный `vitest.integration.config.ts`, **настоящая** PostgreSQL в Docker на порту 5433. Каждый запуск заводит собственную базу `support_bot_test_<метка>_<случайное>` (`globalSetup.ts` создаёт её, накатывает миграции и удаляет после прогона) — так два одновременных запуска не затирают данные друг другу. Адрес базы вычисляется в одном месте: `src/db/__tests__/test-database-url.ts`; если `DATABASE_URL_TEST` задана снаружи (CI), она используется как есть. Переменные окружения задаются в `integration-setup.ts`. Запуск: `pnpm run test:integration` (нужен `docker compose up -d postgres redis`)
