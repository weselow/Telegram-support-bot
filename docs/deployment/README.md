...
### Приём с любых сайтов

Когда виджет ставится на много разных сайтов и вести список неудобно, впишите в `ALLOWED_ORIGINS` отдельную запись `*` — рядом с обычными записями или вместо них:

```
ALLOWED_ORIGINS=*
```

Тогда обращения принимаются с любого сайта. Остальное не меняется: в ответе по-прежнему указывается конкретный источник запроса, а не звёздочка (иначе браузер не отдаст куки сессии), запрос совсем без заголовка `Origin` по-прежнему отклоняется, ограничение частоты обращений по адресу посетителя работает как раньше. При запуске в журнал пишется предупреждение, что дверь открыта.

Что стоит понимать перед тем, как включать:

- проверка домена сдерживает только чужие сайты в браузере. Робот подставляет в заголовке `Origin` любое значение, поэтому от целенаправленного спама список не защищает никогда — от него работает ограничение частоты обращений;
- при открытой двери любой чужой сайт может повесить у себя ваш виджет, и обращения пойдут в вашу группу поддержки. Адрес страницы виден в карточке обращения, так что источник понятен, но разделения по сайтам нет.
