...
| `ALLOWED_ORIGINS` | нет | Домены сайтов с виджетом через запятую, например `example.org,example.net`. Запись разрешает сама себя и свои поддомены, но не родительский домен: `shop.example.org` не открывает `example.org` и `www.example.org` — впишите родительский домен отдельной записью. Кириллический домен можно писать как есть (`сервер-для-1с.рф`) или в ASCII-записи (`xn----1-eddldb4czbcgk3p.xn--p1ai`) — приложение приводит запись к ASCII само. Отдельная запись `*` снимает проверку домена — виджет принимается с любого сайта (см. ниже) |
