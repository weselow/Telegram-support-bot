...
Так работает и сейчас, ничего менять не обязательно. Но в карточке обращения
сотрудник увидит **только домен**, а не страницу: при переходе на чужой домен
браузер укорачивает заголовок `Referer` до адреса домена (политика по умолчанию
`strict-origin-when-cross-origin`).

### Чтобы видеть точную страницу

Адрес страницы надо передать самим — параметром `pageUrl`:

```html
<a href="https://support.yoursite.com/ask-support" class="support-button" data-support-link>
  💬 Нужна помощь?
</a>

<script>
  document.querySelectorAll('a[data-support-link]').forEach(function (link) {
    link.href =
      'https://support.yoursite.com/ask-support?pageUrl=' + encodeURIComponent(location.href);
  });
</script>
```

Адрес обязательно пропускать через `encodeURIComponent` — иначе вопросительный
знак и амперсанды из адреса страницы разорвут ссылку.

Подставить параметр на стороне сервера (шаблонизатором) тоже можно, но
осторожно: если страницы отдаются из кэша (типичный случай для WordPress с
плагином кэширования), в кэш попадёт адрес той страницы, на которой шаблон
собирали в первый раз, и все обращения будут указывать на неё. Скрипт выше от
этого защищён — он подставляет адрес в браузере, уже после выдачи страницы.

### Важно

- Ссылка должна быть обычной `<a>` — переход через `window.open` или
  `location.href` не отправляет `Referer`
- Не добавлять `rel="noreferrer"` и не ставить на страницу
  `<meta name="referrer" content="no-referrer">`. Без `Referer` **параметр
  `pageUrl` тоже отбрасывается** — см. ниже, почему
- HTTPS → HTTPS работает корректно
- Длина адреса в `pageUrl` — до 1000 символов, длиннее не принимается

### Почему параметр проверяется

`pageUrl` приходит из ссылки, то есть его может написать кто угодно. Что в нём
написано — то и увидит сотрудник поддержки в поле «Источник» карточки
обращения, поэтому без проверки это был бы способ показать сотруднику
произвольную ссылку.

Проверка простая: **домен из `pageUrl` должен совпадать с доменом из
`Referer`.** Укороченный `Referer` бесполезен как адрес, но именно как отметку о
домене браузер его и присылает — подделать его со своей страницы нельзя. Если
домены не совпали или `Referer` не пришёл вовсе, параметр отбрасывается и
берётся `Referer`, как раньше.

Сверять со списком `ALLOWED_ORIGINS` здесь смысла нет: этой настройке разрешено
содержать `*` (виджет принимается с любого сайта), и тогда проверка пропустила
бы что угодно.
...
|--------|----------|--------------|
| URL страницы | Параметр `pageUrl`, иначе Referer header | Агентам в тикете |
...
4. Проверка User-Agent (блок ботов)
   ↓
5. Получение IP и адреса страницы (pageUrl или Referer)
   ↓
...
|----------|------------|----------|
| `pageUrl` | нет | Адрес страницы, с которой нажали кнопку, в кодировке `encodeURIComponent`. Принимается, только если его домен совпадает с доменом заголовка `Referer`, схема — `http` или `https`, длина — не больше 1000 символов. Иначе берётся `Referer`. |
...
| `src/http/utils/page-url.ts` | Выбор адреса страницы: параметр `pageUrl` или `Referer` |
