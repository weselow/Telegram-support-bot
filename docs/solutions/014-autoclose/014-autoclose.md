...
- [x] Job для автозакрытия (src/jobs/autoclose.worker.ts):
  - [x] Проверка статуса "Ждём клиента"
  - [x] Закрытие тикета
  - [x] Уведомление в топик
  - [x] Запись события в ticket_events
- [x] Запуск таймера при смене статуса на "Ждём клиента"
- [x] Отмена таймера при ответе клиента
- [x] Напоминание через 24 часа — НЕ ДЕЛАЕМ (см. decisions.md)
- [x] Уведомление клиенту о закрытии
