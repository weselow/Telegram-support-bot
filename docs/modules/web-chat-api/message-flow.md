...
Сам порядок переоткрытия описан один раз — функция `reopenTicket` в
`services/status.service.ts`. Её же зовут личные сообщения и онбординг, поэтому
каналы не могут разойтись в этом месте.
...
└────────────────────────────┘
               ...
               │    direction: USER_TO_SUPPORT,
               │    channel: TELEGRAM,
               │    text + mediaFileId/mediaType/mediaDuration
               │    для вложений
```
...
Тип вложения лежит в `mediaType` (`IMAGE`, `VOICE`, `VIDEO`, `AUDIO`, `DOCUMENT`,
`STICKER`, `ANIMATION`, `VIDEO_NOTE`). По нему история решает, чем рисовать
вложение: `IMAGE` отдаётся полем `imageUrl` и рисуется тегом `img`, `VOICE` —
полем `voiceUrl` и проигрывателем, всё остальное — полем `fileUrl` и ссылкой на
файл. Записи без типа сделаны до его появления в схеме и тоже отдаются ссылкой:
ссылка работает для любого файла. Разбор вложения и построение ссылок собраны в
`src/utils/media.ts` — им пользуются и зеркалирование, и веб-чат.
Длительность в `mediaDuration` заполняется только для голосовых.
