...
```json
{
  "pageUrl": "https://shop.example.com/catalog/item-42",
...
- `pageUrl` — адрес страницы, с которой открыт чат (`window.location.href`). Принимается,
  только если его домен совпадает с заголовком `Origin` запроса, иначе берётся `Referer`.
  Заголовок `Referer` браузер укорачивает до адреса домена при запросе на чужой домен,
  поэтому без `pageUrl` в карточке тикета виден только домен
- Пока топик в группе поддержки не создан, адрес страницы обновляется при каждом вызове —
  в карточке окажется страница, с которой отправлено первое сообщение
